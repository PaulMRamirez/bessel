...
describe('analyze dock state', () => {
  it('defaults to closed on the Access & Coverage tab', () => {
    const store = createAppStore();
    expect(store.getState().analyzeOpen).toBe(false);
    expect(store.getState().analyzeTab).toBe('access');
  });
  ...
  it('selecting a tab opens the dock and sets the active tab', () => {
    const store = createAppStore();
    store.setState(() => ({ analyzeOpen: true, analyzeTab: 'od' }));
    expect(store.getState().analyzeOpen).toBe(true);
    expect(store.getState().analyzeTab).toBe('od');
  });
});
