...
- **F4 — Propagation core (`@bessel/propagator` + `@bessel/astro`).** Analytic
...
### 4.1 Astrodynamics & Orbit Propagation — P1, Phase A, package `@bessel/astro` (+ `@bessel/propagator`)
...
- **PROP-1 (MUST)** Define a `Propagator` interface `propagate(state, etGrid) →
  ...
  The raw transfer DTO lives in `@bessel/spice` (worker layer); `@bessel/astro`
  adapts it. `@bessel/spice` MUST NOT import `@bessel/astro`.
...
RF anchors). 356 tests; `pnpm verify` and `pnpm e2e` green; initial JS and WASM
within budget.
...
Remaining (deeper per-domain features beyond the phase definitions, each its own
focused effort): MCS executor + differential corrector (mission), all-vs-all
screening (conjunction), grid sweep over access (coverage), CK read + pointing
constraints (attitude), time-evolving swaths (sensors), OMM/AEM/CDM + the
automation SDK/BCL (interop), and the cancellable-job worker pool for heavy
sweeps. GMAT remains seam-only per §8.

---

## 10. New packages introduced (summary)

`@bessel/astro`, `@bessel/propagator`, `@bessel/access`, `@bessel/coverage`,
`@bessel/rf`, `@bessel/analysis`, `@bessel/attitude`, `@bessel/sensors`,
`@bessel/conjunction`, `@bessel/events`, `@bessel/sdk`, `@bessel/interop`,
`@bessel/map-projection`, `@bessel/terrain` — all core-layer, depending only on
