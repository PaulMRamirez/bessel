...
| Load an arbitrary mission into the rendered 3D scene | Yes, load and update catalogs at run time | Done | `apps/web/src/generic-mission.ts`, `apps/web/src/engine/engine.ts` (`loadCatalog`, `uploadKernel`), `packages/scene/src/three-scene.ts` (`reset`) | A native catalog now rebuilds the rendered scene generically: catalog bodies, spacecraft, trajectory (sampled in the center frame), and the seven geometry types map to a SceneSpec; the frame loop is mission-driven (no Cassini hardcode); an OPFS kernel-upload path supports unbundled kernels. Verified by `generic-mission.test.ts` and the `generic-mission` e2e (loads a native catalog and re-renders). |
...
The honest remaining work is content (bundle real textures, add a CK fixture)
and surfacing (expose scripting, plugin loading, and the telemetry overlay in
the shell), not missing engine capability.
